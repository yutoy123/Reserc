...
export async function oauthLogin(name, email, provider, providerId) {
  let result = await query(
    "SELECT id, name, email FROM users WHERE oauth_provider = $1 AND oauth_id = $2",
  ...
  if (result.rows.length > 0) {
    ...
    const existing = await query("SELECT id, name, email FROM users WHERE email = $1", [email.toLowerCase()]);
    if (existing.rows.length > 0) {
      ...
      const ins = await query(
        "INSERT INTO users (name, email, oauth_provider, oauth_id) VALUES ($1, $2, $3, $4) RETURNING id, name, email",
        [name || email.split("@")[0], email.toLowerCase(), provider, String(providerId)]
      );
      user = ins.rows[0];
    }
  }
